...
// 컴파일 타임 디버깅 스위치를 위한 #define 들 : 
//#define ADJUSTABLE_LIGHT          // 0번째 광원을 마우스로 조정할 수 있음 (오른쪽 마우스 사용)
#define RENDER_SCENE_LIGHT_POV      // F4 키를 눌러 일반 카메라와 0번째 광원의 시점 (Point-of-view)을 전환
//#define UNCOMPRESSED_VERTEX_DATA  // sdkmesh 파일이 압축되지 않은 정점 데이터를 포함함

// 5가지 렌더링 경로 옵션은, 화면 오른쪽의 라디오 버튼 그룹에 있습니다.
enum DEVICECONTEXT_TYPE
{
    DEVICECONTEXT_IMMEDIATE,                // 단일 스레드, 즉시 컨텍스트 사용
    DEVICECONTEXT_ST_DEFERRED_PER_SCENE,    // 단일 스레드, 씬 단위 디퍼드 컨텍스트 사용
    DEVICECONTEXT_MT_DEFERRED_PER_SCENE,    // 멀티스레드, 씬 단위 디퍼드 컨텍스트 사용
    DEVICECONTEXT_ST_DEFERRED_PER_CHUNK,    // 단일 스레드, 청크 단위 디퍼드 컨텍스트 사용
	DEVICECONTEXT_MT_DEFERRED_PER_CHUNK,    // 멀티스레드, 청크 단위 디퍼드 컨텍스트 사용 ← 이 부분이 가장 고급
};

// 일반적으로 처음 n 개의 광원은 그림자를 투영하며, 그 이후의 광원들은 단순히 조명만 합니다.
const int   g_iNumLights = 4;
const int   g_iNumShadows = 1;
const int   g_iNumMirrors = 4;

// 거울 쿼드의 각 꼭짓점(Vertex)에 대한 구조체입니다.
// 이 구조체에서는 Position(위치)만 사용되며,
// 나머지 요소들은 메인 씬에서 사용하는 정점 셰이더와 동일한 구조를 맞추기 위해 포함되어 있습니다.
struct MirrorVertex
{
	XMFLOAT3 Position;   // 꼭짓점의 위치 (거울의 모서리 위치)
	XMFLOAT3 Normal;     // 법선 벡터 (여기선 사용되지 않지만 셰이더 호환을 위해 포함)
	XMFLOAT2 Texcoord;   // 텍스처 좌표 (마찬가지로 셰이더 일관성을 위해 포함)
	XMFLOAT3 Tangent;    // 탄젠트 벡터 (노멀 매핑 등에 사용될 수 있음)
};
...
// 어떤 장면(그림자/거울/직접 렌더링)을 그릴지에 따라 달라지는,
// 장면 설정(Scene setup)에 필요한 모든 정보를 담고 있음.
//
// 하지만 개별 장면(Scene)마다 매번 바뀌지는 않음.
//
// 이 구조체는 참조(reference) 방식으로 각 청크(Chunk) 단위 워커 스레드에 전달될 수 있음.
// 랜더링 방식 설정!!!
struct SceneParamsStatic
{
    ID3D11DepthStencilState*    m_pDepthStencilState;       // 깊이/스텐실 상태 객체 (렌더링 시 깊이 테스트 및 스텐실 연산 방식 정의)
    UINT8                       m_iStencilRef;              // 스텐실 참조 값 (스텐실 테스트 시 비교 기준값)

    ID3D11RasterizerState*      m_pRasterizerState;         // 래스터라이저 상태 (컬링, 멀티샘플링, 채우기 모드 등을 정의)

    XMFLOAT4                    m_vTintColor;               // 물체에 적용할 색상 틴트 (RGBA 형태)
    XMFLOAT4                    m_vMirrorPlane;             // 거울 평면의 방정식 (Ax + By + Cz + D = 0 형식으로 사용됨)

    // If m_pDepthStencilView is non-nullptr then these 
    // are for a shadow map.  Otherwise, use the DXUT
    // defaults.
    ID3D11DepthStencilView*     m_pDepthStencilView;        // 깊이/스텐실 뷰 포인터 (그림자 맵 렌더링 시 사용됨. nullptr이면 기본값 사용)
    D3D11_VIEWPORT*             m_pViewport;                // 현재 뷰포트 설정 (그림자/거울 등의 장면 별 뷰포트를 다르게 설정 가능)
};

// 어떤 장면(그림자/거울/직접 렌더링)을 그릴지에 따라 필요한,
// 장면 설정(Scene setup)에 필요한 모든 정보를 담고 있으며,
// 또한 장면마다 값이 바뀝니다.
//
// 안전을 위해, 이 구조체는 청크(Chunk) 단위의 워커 스레드에 
// 값(value)으로 전달됩니다. 
// 예를 들어, 메인 스레드는 이미 장면 2를 처리하고 있지만 
// 워커 스레드는 아직 장면 1을 처리하고 있는 경우처럼,
// 장면 간 통신 지연이 생길 수 있기 때문입니다.
// 현재 장면을 렌더링하기위한 카메라 시점 정보
struct SceneParamsDynamic
{
    XMFLOAT4X4 m_mViewProj;
	// View * Projection 행렬.
    // 현재 장면에서 사용하는 카메라의 뷰-프로젝션 변환 행렬입니다.
};

//멀티스레딩 렌더링 작업 큐 시스템에서 각 스레드가 수행할 수 있는 작업의 종류
enum WorkQueueEntryType
{
	WORK_QUEUE_ENTRY_TYPE_SETUP,      // 0. 초기 설정 작업 (예: 뎁스/렌더 상태 설정)
	WORK_QUEUE_ENTRY_TYPE_CHUNK,      // 1. 실제 씬 청크(chunk)를 그리는 작업
	WORK_QUEUE_ENTRY_TYPE_FINALIZE,   // 2. 렌더링 마무리 작업 (예: 결과 머지 또는 후처리)

	WORK_QUEUE_ENTRY_TYPE_COUNT       // 열거형 총 개수 (디버깅이나 배열 크기 등 용도)
};

//작업 큐(Work Queue) 에 들어가는 모든 항목의 기본 구조체입니다.
//실제로는 이 구조체를 기반으로 하여, Setup, Chunk, Finalize 등 각 작업에 필요한 추가 데이터가 들어가는 파생 구조체들이 사용됩니다.
struct WorkQueueEntryBase
{
    WorkQueueEntryType          m_iType;    // 작업 타입(enum) 저장 (Setup, Chunk, Finalize 등)
};
...
HANDLE                      g_hBeginPerSceneRenderDeferredEvent[g_iNumPerSceneRenderThreads];   // Begin 이벤트
HANDLE                      g_hEndPerSceneRenderDeferredEvent[g_iNumPerSceneRenderThreads];     // End 이벤트
                                  ...
                                  float fElapsedTime, void* pUserContext )
{
    ...
    if ( IsRenderMultithreadedPerScene() )  ////// 켜진곳만 보기 확인
    {
    ...
    if ( IsRenderDeferredPerScene() )
    {
        for ( int iInstance = 0; iInstance < g_iNumPerSceneRenderThreads; ++iInstance )
        {
            pd3dImmediateContext->ExecuteCommandList( g_pd3dPerSceneCommandList[iInstance],     // ExecuteCommandList : 명령 목록에 담긴 명령들을 제출, 재생하는 메서드
                !g_bClearStateUponExecuteCommandList );
